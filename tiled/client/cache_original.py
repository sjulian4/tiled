...
def dump(response, content=None):
    ...
    if hasattr(response, "_content"):
        ...
        raise httpx.ResponseNotRead()
    else: # this else statement happens when we are streaming. So it would be when response doesn't have the _content attribute but content has a value
        body = content
        is_stream = True
...
def load(row, request=None):
    ...
    if is_stream:
        ...
        content = body
        stream = None
    response = CachedResponse( # so if CachedResponse has a content of None and stream of httpx.ByteStream(body), then it is streaming
...
def _prepare_database(filepath, readonly):
    Path(filepath).parent.mkdir(parents=True, exist_ok=True) # making a directory
    if readonly:
        ...
        conn = sqlite3.connect( # opens a SQLite connect to the database at filepath
            f"file:{filepath}?mode=ro", uri=True, check_same_thread=False
        )
    else:
        conn = sqlite3.connect(filepath, check_same_thread=False)
    cursor = conn.execute("SELECT name FROM sqlite_master WHERE type='table';") # gets a list of every table in the database
    tables = [row[0] for row in cursor.fetchall()] # the tables, gets all the rows from cursor
    if not tables:
...
class Cache:
    ...
    def write_safe(self):
        ...
        return is_main_thread or sqlite_is_safe # this is just checking if it is safe for multiple threads
...
WHERE cache_key = ?""",
    ...
    WHERE cache_key = ?""",
        ...
        return load(row, request) # so this is returning a loaded cache that can be either for streaming or regular I think??
